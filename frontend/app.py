...
BACKEND_URL = os.getenv(
    "BACKEND_URL", "https://ehsaan08-ai-redrob-ai-hackathon.hf.space"
)
...
cand_file = st.file_uploader(
    "Upload candidate file", type=["jsonl", "json", "csv"], key="cand_file"
)
    ...
    if not st.session_state.jd_text.strip():
        ...
        with st.spinner("⚙️ Streaming, validating, and scoring candidates..."):
            try:
                ...
                r = requests.post(
                    f"{BACKEND_URL}/api/rank", files=files, data=data, timeout=600
                )
                if r.ok:
                    st.session_state.ranking_result = r.json()
                    st.success(
                        f"✅ Evaluated {r.json()['total_evaluated']} valid candidates."
                    )
                else:
...
if result:
    ...
    for cand in result.get("ranked_candidates", []):
        writer.writerow(
            [cand["candidate_id"], cand["rank"], cand["score"], cand["reasoning"]]
        )
